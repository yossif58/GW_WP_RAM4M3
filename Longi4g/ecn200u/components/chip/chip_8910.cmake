...
function(pacvariant_gen)
    ...
    foreach(nvmvariant ${CONFIG_NVM_VARIANTS})
		...
		# Quectel Add
        # set(pac_file_factory ${out_hex_dir}/${BUILD_TARGET}-${nvmvariant}-${BUILD_RELEASE_TYPE}_factory.pac)
        # pac_init_fdl(init_fdl)
        # add_custom_command(OUTPUT ${pac_file_factory}
            # COMMAND python3 ${pacgen_py} ${init_fdl}
				# cfg-erase-flash -i ERASE_BOOT -a ${CONFIG_BOOT_FLASH_ADDRESS} -s ${CONFIG_BOOTLOADER_SIZE}
				# cfg-erase-flash -i ERASE_APP -a ${CONFIG_APP_FLASH_ADDRESS} -s ${CONFIG_APP_FLASH_SIZE} -n
				# cfg-erase-flash -i ERASE_APPIMG -a ${CONFIG_APPIMG_FLASH_ADDRESS} -s ${CONFIG_APPIMG_FLASH_SIZE} -n
				# cfg-erase-flash -i ERASE_FS -a ${CONFIG_FS_SYS_FLASH_ADDRESS} -s ${CONFIG_FS_SYS_FLASH_SIZE} -n
				# cfg-erase-flash -i ERASE_MODEM -a ${CONFIG_FS_MODEM_FLASH_ADDRESS} -s ${CONFIG_FS_MODEM_FLASH_SIZE} -n
				# cfg-erase-flash -i ERASE_FAC -a ${CONFIG_FS_FACTORY_FLASH_ADDRESS} -s ${CONFIG_FS_FACTORY_FLASH_SIZE} -n
                # cfg-nvitem -n "Calibration" -i 0xFFFFFFFF --use 1 --replace 0 --continue 0 --backup 1
                # cfg-nvitem -n "GSM Calibration" -i 0x26d --use 1 --replace 0 --continue 1 --backup 1
                # cfg-nvitem -n "LTE Calibration" -i 0x26e --use 1 --replace 0 --continue 0 --backup 1
                # cfg-nvitem -n "IMEI" -i 0xFFFFFFFF --use 1 --replace 0 --continue 0 --backup 0
                # cfg-nvitem -n "BT_Config" -i 0x191 --use 1 --replace 0 --continue 1 --backup 1
                # cfg-image -i AP -a ${CONFIG_APP_FLASH_ADDRESS} -s ${CONFIG_APP_FLASH_SIZE}
                    # -p ${out_hex_dir}/${BUILD_TARGET}.sign.img
                # ${cfg_ap2}
                ...
                # cfg-nv -s ${CONFIG_NVBIN_FIXED_SIZE} -p ${out_hex_dir}/${nvmvariant}_nvitem.bin
                # cfg-pack-cpio -i PREPACK -p ${prepack_cpio}
                # dep-gen --base ${BINARY_TOP_DIR} ${pac_file_factory} ${pac_file_factory}.d
                # pac-gen ${pac_file_factory}
            # DEPFILE ${pac_file_factory}.d
            # DEPENDS ${pac_fdl_files} ${out_hex_dir}/boot.sign.img
                # ${out_hex_dir}/${BUILD_TARGET}.sign.img
                # ${out_hex_dir}/${nvmvariant}.img
                # ${target_flash2_bin}
            # WORKING_DIRECTORY ${SOURCE_TOP_DIR}
        # )
        # add_custom_target(${nvmvariant}_factory_pacgen ALL DEPENDS ${pac_file_factory})
		# Quectel End
    endforeach()
endfunction()
