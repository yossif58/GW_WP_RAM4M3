...
#include "longi4g.h"
#include "longi_uart.h"
//#include "ql_virt_at_demo.h"
#ifdef QL_APP_FEATURE_UART
#include "longi_uart.h"
// y.f.include instad longi_uart.h #include "uart_demo.h"
#endif
...
#include "mqtt_demo.h"
#endif
#ifdef QL_APP_FEATURE_FIREWALL
#include "firewall_demo.h"
#endif
...
/*Caution:If the macro of secure boot and the function are opened, download firmware and restart will enable secure boot.
    ...
    ql_gpioint_app_init();
#endif
	ql_longi_uart_app_init();
	ql_Longi4G_app_init();         // y.f. 7/5/23 add logi4g task
   // ql_embed_nor_flash_app_init(); // y.f 14/5/23 add for internal configuration in Flash
...
#ifdef QL_APP_FEATURE_VIRT_AT
   //y.f removed  ql_virt_at_app_init();
#endif
...
#ifdef QL_APP_FEATURE_EMBED_NOR_FLASH
   // y.f. removed put instad   ql_embed_nor_flash_app_init();  // y.f. 15/5/23 
 // y.f. temp removed no need for task   embed_nor_flash_app_init();
#endif
