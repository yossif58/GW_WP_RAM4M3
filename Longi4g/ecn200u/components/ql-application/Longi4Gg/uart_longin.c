...
void ql_debug_uart_notify_cb(unsigned int ind_type, ql_uart_port_number_e port, unsigned int size)
{
    ...
    int read_len = 0;
	uint8_t *rxtmpbuf = malloc (sizeof (uint8_t) * 1024);
	uint16_t tmplen;
    ...
    switch(ind_type)
    {
        ...
        case QUEC_UART_RX_RECV_DATA_IND:
        {
            ...
            while(size > 0)
            {
				...
				memcpy ((uint8_t*)&rxtmpbuf[tmplen],recv_buff,read_len);
				tmplen+=read_len;
			...
			if (tmplen)
				Send2DebugUart(rxtmpbuf, tmplen); // lbk temp to be removed 
            break;
        }
...
void ql_uart_meter_notify_cb(unsigned int ind_type, ql_uart_port_number_e port, unsigned int size)
{
    ...
    unsigned int real_size = 0;
    int read_len = 0;
	uint8_t *rxtmpbuf = malloc (1024 *sizeof(uint8_t));
    uint16_t InfoMetelen;
    QL_UART_DEMO_LOG("UART port %d receive ind type:0x%x, receive data size:%d", port, ind_type, size);
    switch(ind_type)
    {
        case QUEC_UART_RX_OVERFLOW_IND:  //rx buffer overflow
        case QUEC_UART_RX_RECV_DATA_IND:
        {
            ...
            while(size > 0)
            {
                ...
                real_size= MIN(size, QL_UART_RX_BUFF_SIZE);
                read_len = ql_uart_read(port, recv_buff, real_size);
				memcpy ((uint8_t*)&rxtmpbuf[InfoMetelen],recv_buff,read_len);
				InfoMetelen+=read_len;
                QL_UART_DEMO_LOG("read_len=%d, recv_data=%s", read_len, recv_buff);
                if((read_len > 0) && (size >= read_len))
                {
                    size -= read_len;
			...
			if (InfoMetelen)
			{
				Meter_Handler((uint8_t *)rxtmpbuf,InfoMetelen,DATA_IN_EVENT); 
				Send2DebugUart(rxtmpbuf, InfoMetelen); 
			}
            break;
        }
        case QUEC_UART_TX_FIFO_COMPLETE_IND: 
...
static void ql_debug_uart_thread(void *param)
{
    ...
    MeterQueueStruct *InfoDebug = malloc (sizeof(MeterQueueStruct ));
    /***********************************************************
	Note start:
	...
	************************************************************/
	...
	if(QL_UART_SUCCESS == ret)
	{
	    ...
	    while(1)
	    {
	    	...
	    	ql_rtos_queue_wait (DebugUartqueue ,(uint8_t*)&InfoDebug, 1024, QL_WAIT_FOREVER);
			if(InfoDebug->len)
			{
				ql_uart_write(QL_UART_PORT_2, (uint8_t*)InfoDebug->rcvbuf, InfoDebug->len);
	...
	************************************************************/
	...
	if(QL_UART_SUCCESS == ret)
	{
	    ...
	    while(1)
	    {
		    ...
		    ql_rtos_queue_wait (MeterUartqueue ,(uint8_t*)&InfoMeter, 1024, QL_WAIT_FOREVER);
			ql_uart_write(QL_UART_PORT_1, (uint8_t*)InfoMeter.rcvbuf, InfoMeter.len);
