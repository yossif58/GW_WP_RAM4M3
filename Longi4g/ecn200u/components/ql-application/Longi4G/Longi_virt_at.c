...
extern ql_queue_t GSMQueue;
GsmQueueStruct Info;
char str [1024];
extern ql_queue_t DebugUartqueue;
extern ql_timer_t AT_debounce_timer;
...
void ql_virt_at0_notify_cb(unsigned int ind_type, unsigned int size)
{
    ...
    if(QUEC_VIRT_AT_RX_RECV_DATA_IND == ind_type)
    {
		...
		Gsm_event_in =true;
		memcpy(	(uint8_t*)&GsmDatain[Gsmlen],(uint8_t*)recv_buff,strlen((const char*)recv_buff));	
		Gsmlen = strlen((const char*)recv_buff);
	   	ql_rtos_timer_start(AT_debounce_timer, 100, 1); // start y.f. timer for end of chars  
    }
    free(recv_buff);
    recv_buff = NULL;
}
...
static void ql_virt_at_demo_thread(void *param)
{
   ...
   // char *cmd2 = "at+qdbgcfg=\"oem\"\r\n";
    ...
    {     
    	...
    	ql_rtos_queue_wait (GSMQueue , (uint8_t*)&Info, 1024, QL_WAIT_FOREVER);
		ql_virt_at_write(QL_VIRT_AT_PORT_0,(uint8_t*)Info.rcvbuf, Info.len);   
    }
exit:    
...
void ql_virt_at_app_init()
{
	...
	sprintf (str ,"created demo virtual at" );
    ql_rtos_queue_release(DebugUartqueue, 20, (uint8 *)str, QL_NO_WAIT);

//	Send2DebugUart((uint8_t *)str, strlen(str));     
}
#if 0
void SendData2Modem(uint8_t * pDdataIn, uint16_t len)
{
	clrMeterEvent();
    sprintf (str ,(const char *)pDdataIn,len );
    Send2DebugUart((uint8_t *)str, len);  
	ql_virt_at_write(QL_VIRT_AT_PORT_0,(uint8_t*)pDdataIn, len);   
