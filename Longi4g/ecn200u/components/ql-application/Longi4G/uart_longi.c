...
#pragma pack(1)
extern bool meter_event_in;
uint16_t meterlen; 
uint8_t  MeterDatain [1024];
...
extern ql_timer_t Uart_debounce_timer;
#define UART_DEBOUNCE_TIME 100
//extern bool Meter_Handler(uint8_t * pDdataIn, uint16_t Len , EventType Event);
void SendData2Meter(uint8_t * DataIn, uint16_t Len,bool display);
void Send2DebugUart(uint8_t *pDdataIn ,uint32_t len);
    ...
    switch(ind_type)
    {
        ...
        case QUEC_UART_RX_RECV_DATA_IND:
        {
            ...
            while(size > 0)
            {
				...
				memcpy((uint8_t*)&InfoDebug.rcvbuf[InfoDebug.len],recv_buff,read_len);
				InfoDebug.len+=read_len;
...
//			Send2DebugUart(bufin ,buflen);
			ql_rtos_queue_release(DebugUartqueue, sizeof(long long), (uint8 *)&InfoDebug, QL_NO_WAIT); //lbk
...
{
    unsigned char *recv_buff = calloc(1, QL_UART_RX_BUFF_SIZE+1);
//	unsigned char *str1 = calloc(1, 1024);
    unsigned int real_size = 0;
    int read_len = 0;
    QL_UART_DEMO_LOG("UART port %d receive ind type:0x%x, receive data size:%d", port, ind_type, size);
    switch(ind_type)
    {
        ...
        case QUEC_UART_RX_RECV_DATA_IND:
        {
            ...
            while(size > 0)
            {
                ...
                read_len = ql_uart_read(port, recv_buff, real_size);
	//			if (meter_event_in==false)  // y.f. 31/08/22
				{					
					memcpy ((uint8_t*)&MeterDatain[meterlen],recv_buff,read_len);
					meterlen+=read_len;
					QL_UART_DEMO_LOG("read_len=%d, recv_data=%s", read_len, recv_buff);
					if((read_len > 0) && (size >= read_len))
					{
			...
			//meter_event_in =true;
			ql_rtos_timer_start(Uart_debounce_timer, UART_DEBOUNCE_TIME, 1); // start y.f. timer for end of chars  
	
		//	Send2DebugUart(MeterDatain, meterlen);
		//	Meter_Handler(MeterDatain,meterlen,DATA_IN_EVENT); 
		//	ql_rtos_queue_release(DebugUartqueue, meterlen, (uint8 *)MeterDatain, QL_NO_WAIT);
		//	Send2DebugUart(DataIn,meterlen);
	...
	************************************************************/
	...
	if(QL_UART_SUCCESS == ret)
	{
		...
		//		write_len = ql_uart_write(QL_UART_PORT_2, recvMsg, strlen((char *)recvMsg));
				write_len = ql_uart_write(QL_UART_PORT_2, recvMsg, 25);  // temp 20
	...
	************************************************************/
	...
	if(QL_UART_SUCCESS == ret)
	{
	    ...
	    while(1)
	    {
			...
			ql_rtos_queue_wait (MeterUartqueue ,buf, sizeof(buf), QL_WAIT_FOREVER);
			MeterQueueStruct  *pMeterQueue = (MeterQueueStruct*)buf;
			ql_uart_write(QL_UART_PORT_1, (uint8_t*) &pMeterQueue->rcvbuf[0], pMeterQueue->len); 
...
void Send2DebugUart(uint8_t *pDdataIn ,uint32_t len)
{
	...
	ql_uart_tx_status_e tx_status;
	ql_uart_write(QL_UART_PORT_2, pDdataIn, len);
	ql_uart_get_tx_fifo_status(QL_UART_PORT_2, &tx_status);
...
void SendData2Meter(uint8_t * pDdataIn, uint16_t len,bool display)
{
	if (display)
	{
		char str[100] = {0};
	...
	ql_uart_tx_status_e tx_status;
	ql_uart_write(QL_UART_PORT_1, pDdataIn, len);
	ql_uart_get_tx_fifo_status(QL_UART_PORT_1, &tx_status);
	ql_rtos_task_sleep_ms(20);  	
	if (display)
	{
		Send2DebugUart((uint8_t *)pDdataIn, len);  // Y.F. ADD temp we have to be removed 
		ql_rtos_task_sleep_ms(20);
	}
}
