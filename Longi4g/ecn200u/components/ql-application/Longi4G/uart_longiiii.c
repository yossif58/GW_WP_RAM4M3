    ...
    switch(ind_type)
    {
        ...
        case QUEC_UART_RX_RECV_DATA_IND:
        {
            ...
            while(size > 0)
            {
				...
				memcpy((uint8_t*)&bufin[buflen],recv_buff,read_len);
				buflen+=read_len;
    ...
    switch(ind_type)
    {
        ...
        case QUEC_UART_RX_RECV_DATA_IND:
        {
            ...
            while(size > 0)
            {
                ...
                read_len = ql_uart_read(port, recv_buff, real_size);
				len+=read_len;
				memcpy ((uint8_t*)&DataIn[len],recv_buff,read_len);
                QL_UART_DEMO_LOG("read_len=%d, recv_data=%s", read_len, recv_buff);
                if((read_len > 0) && (size >= read_len))
                {
                    size -= read_len;
	...
	************************************************************/
	...
	if(QL_UART_SUCCESS == ret)
	{
	    ...
	    while(1)
	    {
			...
			ql_rtos_queue_wait (MeterUartqueue , (uint8_t*)&MeterInfoIn, 1024, QL_WAIT_FOREVER);
			ql_uart_write(QL_UART_PORT_1, MeterInfoIn.rcvbuf, MeterInfoIn.len);  // temp 20
