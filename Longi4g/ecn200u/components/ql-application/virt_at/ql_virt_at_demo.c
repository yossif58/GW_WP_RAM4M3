...
y.f. i am not using demo it was removed to longi4g dir 
/*===========================================================================
 * Macro Definition
 ===========================================================================*/
...
void ql_virt_at0_notify_cb(unsigned int ind_type, unsigned int size)
{
    ...
    if(QUEC_VIRT_AT_RX_RECV_DATA_IND == ind_type)
    {
		...
		Gsm_Handler((uint8_t*)recv_buff ,real_size , DATA_IN_EVENT);
        ql_rtos_task_sleep_ms(10);		
		sprintf (str ,"Rx virtual at from modem\r\n" );
    	Send2DebugUart((uint8_t *)str, strlen(str));  
        ql_rtos_task_sleep_ms(10);
	    Send2DebugUart((uint8_t *) recv_buff, real_size);        
