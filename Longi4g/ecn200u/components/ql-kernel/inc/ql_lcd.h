...
typedef struct
{
    uint32_t width;       //currently only 480 are supported
    uint32_t height;      //currently only 854 are supported
} ql_mipi_lcd_info_t;
...
/**
 * @brief Free MIPI LCD memory space
 * @return
 *      QL_LCD_SUCCESS
 */
ql_errcode_lcd ql_mipi_lcd_free(void);
 ...
 *      QL_LCD_SUCCESS
 */
ql_errcode_lcd ql_mipi_lcd_write_cmd(void *cmd_data);
